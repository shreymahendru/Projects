...
/**
 * Clear the listcountist of monitored pids for a specific syscall.
 */
 ...
 *     ax, bx, cx, dx, si, d   i, and bp registers (see the pt_regs struct).
 * - Don't forget to call the original system call, so we allow processes to proceed as normal.
 */
asmlinkage long interceptor(struct pt_regs reg) {

	//check_pid_monitored BY EVERY THING
	if (table[reg.ax].monitored != 2)
	{
		//CHECK PID IN THE LIST
		int moniter= check_pid_monitored(reg.ax, current->pid);
		if(moniter == 1)
			{	// LOG MSG
	...
	{
		// IF EVERY THING IS MONITORED
		log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
	}
	//CALLL THE ORIGINAL SYSCALL
	return table[reg.ax].f(reg);

	// if monitered the log msg else dont 
	//log msg 
	//call the original shit
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
    ...
    if(cmd == REQUEST_STOP_MONITORING || cmd == REQUEST_START_MONITORING)
	{
		...
		//CHECK PID IS 0 OR NOT
		if(pid != 0)
		{
			// if (table[syscall].monitored == 2)
			// {
			// 	//EVERY THING IS ALREADY MONITORED
			// 	return -EINVAL;
			// }
			//PID IS NOT 0, CHECK THE VALIDITY OF PID
			...
			{
				//PID IS VALID BUT NOT 0
				//NOW CHECK IF THE CALLING PROCESS IS THE ROOT OR NOT
				if(current_uid() != 0)
				{
					// CALLING PROCESS IS NOT THE ROOT
					//CHECK IF THE THE PID IS OWNED BY THE CALLING PROCESS
				...
				//OWNED BY THE PROCESS OR IS THE ROOT
        ...
        else if (pid == 0)
        {
            ...
            if(cmd == REQUEST_START_MONITORING)
            {
                if(table[syscall].monitored == 2)
                {
                    //EVERYTHING IS MONITORED ALREADY
                    return -EBUSY;
                }
                spin_lock(&pidlist_lock);
