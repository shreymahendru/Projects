...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		unsigned int vic_frame= victim->frame;
		off_t vic_offset= victim->swap_off;
		if(vic_frame & PG_DIRTY) //if dirty 
		{
			victim->swap_off= swap_pageout(vic_frame >> PAGE_SHIFT, vic_offset); //frame is swapper out
			victim->frame= victim->frame | PG_ONSWAP; //PUT FRAME ON SWAP
			victim->frame= (victim->frame | PG_VALID) ^ PG_VALID; //SET FRAME TO INVALID
			evict_dirty_count++;
		}
		else //not dirty
		{
			victim->frame= victim->frame | PG_ONSWAP; //frame is on swap
			victim->frame= (victim->frame | PG_VALID) ^ PG_VALID;// frame is invalid
			evict_clean_count++; //Not dirty 
...
char *find_physpage(addr_t vaddr, char type) {
	...
	uintptr_t entry = (pgdir[idx].pde | 1) ^ 1; // get 0 at the end of the address 

	pgtbl_entry_t *sec_level = (pgtbl_entry_t*)entry;
	// Use vaddr to get index into 2nd-level page table and initialize 'p'
	p = &sec_level[PGTBL_INDEX(vaddr)];
